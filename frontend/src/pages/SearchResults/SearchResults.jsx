...
const SearchResults = () => {
  ...
  const fetchInitialResults = useCallback(async () => {
    ...
    if (data.filters?.brands && data.filters.brands.length > 0) {
          setAvailableBrands(data.filters.brands);
        } else {
          // Build brands from actual product stores
          const storeMap = new Map();
          data.products.forEach(p => {
            if (p.store) {
              const id = p.store.toLowerCase().replace(/\s+/g, '-');
              storeMap.set(id, { id, name: p.store, count: (storeMap.get(id)?.count || 0) + 1 });
            }
          });
          if (storeMap.size > 0) setAvailableBrands(Array.from(storeMap.values()));
        }
